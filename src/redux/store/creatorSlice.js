...
const creatorSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchAsyncCreators.pending, (state) => {
        state.creatorsStatus = STATUS.LOADING;
      })
      .addCase(fetchAsyncCreators.fulfilled, (state, action) => {
        state.creators = action.payload;
        state.creatorsStatus = STATUS.SUCCEEDED;
      })
      .addCase(fetchAsyncCreators.rejected, (state) => {
        state.creatorsStatus = STATUS.FAILED;
      });
  }, // This is where the builder callback is used within the extraReducers property
});
